...
interface CalendarSectionProps {
  initialGoogleStatus?: CalendarStatus
  initialMicrosoftStatus?: CalendarStatus
}

function CalendarSection({ initialGoogleStatus, initialMicrosoftStatus }: CalendarSectionProps) {
  const [googleStatus, setGoogleStatus] = useState<CalendarStatus | null>(initialGoogleStatus ?? null)
  const [microsoftStatus, setMicrosoftStatus] = useState<CalendarStatus | null>(initialMicrosoftStatus ?? null)
  ...
  // Only fetch client-side when no server-side data was provided.
  useEffect(() => {
    if (initialGoogleStatus === undefined || initialMicrosoftStatus === undefined) {
      void fetchStatus()
    }
  }, [fetchStatus, initialGoogleStatus, initialMicrosoftStatus])
  ...
  // Loading skeleton — only shown when no initial data and fetch hasn't returned yet
...
export interface SettingsClientProps {
  initialBillingData?: UsageData | null
  initialGoogleStatus?: CalendarStatus
  initialMicrosoftStatus?: CalendarStatus
}
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-border p-6">
          ...
          <CalendarSection
            ...
            initialMicrosoftStatus={initialMicrosoftStatus}
          />
        </div>
        ...
        <div className="bg-white rounded-xl border border-border p-6">
          ...
          <BillingSection initialData={initialBillingData} />
        </div>
      </div>
    </div>
  )
